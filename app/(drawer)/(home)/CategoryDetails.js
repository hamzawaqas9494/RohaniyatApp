...
// src/screens/CategoryDetails.js
...
export default function CategoryDetails() {
  ...
  const loadData = async () => {
    ...
    setLoading(true);
    const found = await fetchTableData(tableName, { id, forceRefresh: !isOffline });
    if (found) {
      setItem(found);
      navigation.setOptions({ title: found.title });
    } else {
  ...
  useEffect(() => {
    if (isFocused && tableName && id) {
      loadData();
    }
  }, [isFocused, id, tableName, isOffline]);
