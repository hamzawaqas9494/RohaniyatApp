...
export default function ItemList() {
  ...
  const navigation = useNavigation();
  const { tableName, categoryId, subcategoryId, categoryLabel ,label} = route.params || {}; // نیا: categoryId اور subcategoryId
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        // نیا سسٹم: tableName + categoryId + subcategoryId سے آئٹمز لے رہے ہیں
        const data = await fetchItems(tableName, categoryId, subcategoryId);
        //  console.log(tableName, categoryId, subcategoryId,"tableName, categoryId, subcategoryId itemslist");
